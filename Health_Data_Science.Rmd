...
#we will choose some variables to keep

vars_to_keep = c("ADSMOK42","SDENICPROD","OFTSMK53","ADDPRS42","ADGENH42",
                     "ADENGY42","ADHOPE42","ADNERV42","ADPCFL42","ADSAD42","AGELAST",
                     "ASTHDX","CALUNG","CHOLDX","DIABDX_M18","EMPHDX","FAMINC21",
                      "HIBPDX","MNHLTH31","RTHLTH31",
                     "TTLP21X","FOODVL21","WLKLIM53","SDCOMPAN","SDLEFTOUT","SDLIFE")

small_df = h233 %>% select(all_of(vars_to_keep))

small_df

small_df %>% select(ADSMOK42, ADGENH42, ADENGY42) %>% group_by(ADSMOK42) %>%
  summarise(n_excellent_health = mean(ADGENH42==1))
