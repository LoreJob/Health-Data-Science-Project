...
```{r}
create_plots_demogr <- function(data, demographic_variables) {
  data <- data %>%
    mutate(
    ...
    filter(!is.na(tot_inc_group) & !is.na(fam_inc_group) & !is.na(SEX)) %>%
  ...
  create_plots <- function(data, variable) {
    ...
    barplot(table(data[[variable]]), main = variable, xlab = variable, ylab = "Frequency", las = 2, cex.names = 0.8, col=my_colors)
  }
  
  par(mfrow = c(1, 3)) 
  for (variable in demographic_variables) {
    create_plots(data, variable)
  }
}

# Plot for smokers
create_plots_demogr(smokers_df, demographic_variables)

# Plot for non-smokers
create_plots_demogr(non_smokers_df, demographic_variables)
```
