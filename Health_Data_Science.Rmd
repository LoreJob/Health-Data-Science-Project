...
```{r}
calculate_and_plot <- function(data, age_column, smoking_column, health_disease_variables, condition_labels) {
  data_filtered <- data %>% filter(!!sym(age_column) >= 18)
  total_count <- nrow(data_filtered)
  result_df <- data.frame(Condition = character(), n_smokers_with_condition = integer(), perc_smokers = numeric(), stringsAsFactors = FALSE)

  for (condition in health_disease_variables) {
    summary_df <- data_filtered %>%
      summarise(
        n_smokers_with_condition = sum(!!sym(smoking_column) == "1 YES" & !!sym(condition) == "1 YES", na.rm = TRUE),
        total_with_condition = sum(!!sym(condition) == "1 YES", na.rm = TRUE)
      ) %>%
      mutate(
        perc_smokers = 100 * n_smokers_with_condition / total_count,
        Condition = condition
      )
    result_df <- rbind(result_df, summary_df)
  }
  
  result_df$Condition <- factor(result_df$Condition, levels = health_disease_variables, labels = condition_labels)
  
  ggplot(result_df, aes(x = Condition, y = perc_smokers, fill = Condition)) +
    geom_col() +
    coord_flip() +
    labs(
      title = "Percentage of Persons 18 or Older \nWho Report Specific Health Diseases and Continue to Smoke",
      x = "Heath Diseases",
      y = "Percentage"
    ) +
    theme_classic() +
...
calculate_and_plot(small_df, age_column = "AGELAST", smoking_column = "ADSMOK42", health_disease_variables, condition_labels)
```
...
calculate_and_plot <- function(data, age_column, smoking_column, health_disease_variables, condition_labels) {
  ...
  for (condition in health_disease_variables) {
    summary_df <- data_filtered %>%
      summarise(
        n_smokers_with_condition = sum(!!sym(smoking_column) == "2 NO" & !!sym(condition) == "1 YES", na.rm = TRUE),
        total_with_condition = sum(!!sym(condition) == "1 YES", na.rm = TRUE)
      ) %>%
      mutate(
        perc_smokers = 100 * n_smokers_with_condition / total_count,
...
## Contingency Tables with smoking
...
```{r}
#the above graph is equivalent to this one
...
### Contingency tables for general health and smoking
```{r}
# variables about general health
# "" "" "" "WLKLIM53"
...
### Electronic nicotine product usage and general health indicators
...
### Mental health and electronic nicotine product usage
```{r}
...
```
```{r}
income_mental_health = small_df %>% mutate(income_group = cut(FAMINC21,
                                    breaks = c(-Inf,36000,90000, Inf),
                  labels = c("low","medium","high"),
                  right = FALSE)) %>% select(income_group, ADSMOK42, SDENICPROD,MNHLTH31)


table_multi_cat_income = function(data, explanatory_var, var_of_interest,
                                  group, cond_prob = T){
  tbl = data %>% filter(income_group == group) %>%
    select(explanatory_var, var_of_interest,income_group)%>%
    filter(.data[[explanatory_var]] %in% c("1 YES", "2 NO"),
           !.data[[var_of_interest]] %in% c("-15 CANNOT BE COMPUTED",
                                            "-1 INAPPLICABLE",
                                            "-8 DK","-7 REFUSED"))%>%
...
table_multi_cat_income(income_mental_health, "ADSMOK42", "MNHLTH31","low",F)
