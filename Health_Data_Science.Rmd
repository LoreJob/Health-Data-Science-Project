...
diseases_percentages %>% filter(ADSMOK42 != "-1 INAPPLICABLE") %>%
  ggplot(aes(x = condition, y = percentage*100, fill = condition)) +
    ...
    coord_flip() +
    labs(
      title = "Percentage of people 18 or Older Who Report Specific Health Diseases",
      x = "",
      y = "Percentage"
    ) +
  facet_wrap(~ADSMOK42,
             labeller = labeller(category = c("1 YES" = "Smokers", "2 NO" = "non-Smokers")))+
    theme_classic() +
    theme(legend.position = "none")
...
ggplot(chisq_df_diseases, aes(x = name, y = value)) +
  ...
  scale_x_discrete(labels = c("p_val_full_data" = "All patients",
                              "p_val_65plus_data" = "ages 45-64",
                              "p_val_45_64_data" = "ages 65 plus")) +
...
table_multi_cat(small_df, "SDENICPROD", "WLKLIM53") %>% as_tibble() %>%
  ...
  labs(title = "Contingency table: Electronic nicotine product usage and Limitation in Physical Functioning", x = "",
       y="frequency", fill="") +
  scale_x_discrete(labels = c("1 YES" = "users",
                              "2 NO" = "Non users"))
