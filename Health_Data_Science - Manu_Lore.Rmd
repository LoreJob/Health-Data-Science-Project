...
filter_df <-small_df %>% select(CALUNG, ADSMOK42, SEX, age_group) %>% filter(rowSums(sapply(., function(x) x %in% values_to_remove)) == 0)

# filter_df <- filter_df %>% filter(age_group == "65+")

filter_df$CALUNG <- relevel(filter_df$CALUNG, ref = "1 YES")
...
fit <- glm(CALUNG ~ ADSMOK42 * SEX * age_group, data = filter_df, family = "binomial")
...
### Asthma 
```{r}
filter_df <-small_df %>% select(ASTHDX, ADSMOK42, SEX, age_group) %>% filter(rowSums(sapply(., function(x) x %in% values_to_remove)) == 0)
...
fit <- glm(ASTHDX ~ ADSMOK42 * SEX * age_group, data = filter_df, family = "binomial")
...
filter_df <-small_df %>% select(ARTHDX, ADSMOK42, SEX, age_group) %>% filter(rowSums(sapply(., function(x) x %in% values_to_remove)) == 0)
...
fit <- glm(ARTHDX ~ ADSMOK42 * SEX * age_group, data = filter_df, family = "binomial")
...
```{r}
filter_df <-small_df %>% select(CHOLDX, ADSMOK42, SEX, age_group) %>% filter(rowSums(sapply(., function(x) x %in% values_to_remove)) == 0)
...
fit <- glm(CHOLDX ~ ADSMOK42 * SEX, data = filter_df, family = "binomial")
...
### Chronic Bronchitis
```{r}
filter_df <-small_df %>% select(CHBRON31, ADSMOK42, SEX, age_group) %>% filter(rowSums(sapply(., function(x) x %in% values_to_remove)) == 0)
...
fit <- glm(CHBRON31 ~ ADSMOK42 * SEX * age_group, data = filter_df, family = "binomial")
...
```{r}
filter_df <-small_df %>% select(DIABDX_M18, ADSMOK42, SEX, age_group) %>% filter(rowSums(sapply(., function(x) x %in% values_to_remove)) == 0)
...
fit <- glm(DIABDX_M18 ~ ADSMOK42 * SEX * age_group, data = filter_df, family = "binomial")
...
```{r}
filter_df <-small_df %>% select(EMPHDX, ADSMOK42, SEX, age_group) %>% filter(rowSums(sapply(., function(x) x %in% values_to_remove)) == 0)
...
fit <- glm(EMPHDX ~ ADSMOK42 * SEX * age_group, data = filter_df, family = "binomial")

summary(fit)
```

### High Blood Pressure (need to discuss)
```{r}
filter_df <-small_df %>% select(HIBPDX, ADSMOK42, SEX, age_group) %>% filter(rowSums(sapply(., function(x) x %in% values_to_remove)) == 0)
...
fit <- glm(HIBPDX ~ ADSMOK42 * SEX * age_group, data = filter_df, family = "binomial")
...
### Stroke or Transient Ischemic Attack (need to discuss)
```{r}
filter_df <-small_df %>% select(STRKDX, ADSMOK42, SEX, age_group) %>% filter(rowSums(sapply(., function(x) x %in% values_to_remove)) == 0)
...
fit <- glm(STRKDX ~ ADSMOK42 * SEX * age_group, data = filter_df, family = "binomial")
...
```{r}
filter_df <-small_df %>% select(ADDPRS42, ADSMOK42, SEX, age_group) %>% filter(rowSums(sapply(., function(x) x %in% values_to_remove)) == 0)
...
fit <- glm(ADDPRS42 ~ ADSMOK42 * SEX * age_group, data = filter_df, family = "binomial")
...
### Feeling Hopeless
```{r}
filter_df <-small_df %>% select(ADHOPE42, ADSMOK42, SEX, age_group) %>% filter(rowSums(sapply(., function(x) x %in% values_to_remove)) == 0)
...
fit <- glm(ADHOPE42 ~ ADSMOK42 * SEX * age_group, data = filter_df, family = "binomial")
...
```{r}
filter_df <-small_df %>% select(ADNERV42, ADSMOK42, SEX, age_group) %>% filter(rowSums(sapply(., function(x) x %in% values_to_remove)) == 0)
...
fit <- glm(ADNERV42 ~ ADSMOK42 * SEX * age_group, data = filter_df, family = "binomial")
...
```{r}
filter_df <-small_df %>% select(ADPCFL42, ADSMOK42, SEX, age_group) %>% filter(rowSums(sapply(., function(x) x %in% values_to_remove)) == 0)
...
fit <- glm(ADPCFL42 ~ ADSMOK42 * SEX * age_group, data = filter_df, family = "binomial")
...
```{r}
filter_df <-small_df %>% select(ADSAD42, ADSMOK42, SEX, age_group) %>% filter(rowSums(sapply(., function(x) x %in% values_to_remove)) == 0)
...
fit <- glm(ADSAD42 ~ ADSMOK42 * SEX * age_group, data = filter_df, family = "binomial")
...
```{r}
filter_df <-small_df %>% select(MNHLTH31, ADSMOK42, SEX, age_group) %>% filter(rowSums(sapply(., function(x) x %in% values_to_remove)) == 0)
...
fit <- glm(MNHLTH31 ~ ADSMOK42 * SEX * age_group, data = filter_df, family = "binomial")
...
## Trying with how often do you smoke
...
filter_df <-small_df %>% select(CALUNG, OFTSMK53, SEX, age_group) %>% filter(rowSums(sapply(., function(x) x %in% values_to_remove)) == 0)

# filter_df <- filter_df %>% filter(age_group == "65+")

filter_df$CALUNG <- relevel(filter_df$CALUNG, ref = "1 YES")
filter_df$OFTSMK53 <- relevel(filter_df$OFTSMK53, ref = "3 NOT AT ALL")
filter_df$SEX <- relevel(filter_df$SEX, ref = "1 MALE")
filter_df$age_group <- relevel(filter_df$age_group, ref = "65+")

fit <- glm(CALUNG ~ OFTSMK53 * SEX * age_group, data = filter_df, family = "binomial")
...
### Chronic Bronchitis
```{r}
filter_df <-small_df %>% select(CHBRON31, OFTSMK53, SEX, age_group) %>% filter(rowSums(sapply(., function(x) x %in% values_to_remove)) == 0)
...
fit <- glm(CHBRON31 ~ OFTSMK53 * SEX * age_group, data = filter_df, family = "binomial")
