...
fit <-  vglm(MNHLTH31 ~ AGELAST + ADSMOK42,
...
```{r}
df_pred = data.frame(AGELAST = seq(min(filter_df$AGELAST), max(filter_df$AGELAST), l = 500), ADSMOK42 = "2 NO")
...
```{r}
fit2 <-  vglm(MNHLTH31 ~ ADSMOK42 + SEX + AGELAST,
             family = cumulative(parallel = TRUE),
             data = filter_df)
...
<<<<<<< HEAD
df_pred = data.frame(AGELAST = seq(min(filter_df$AGELAST), max(filter_df$AGELAST), l = 50), ADSMOK42 = "2 NO", SEX = "1 MALE")
=======

```{r}
df_pred = data.frame(AGELAST = seq(min(filter_df$AGELAST), max(filter_df$AGELAST), l = 50), ADSMOK42 = "2 NO", SEX = "2 FEMALE")
>>>>>>> 651c958df8e6ad1a19306cceb76286b2b935710c
...
matplot(df_pred$AGELAST, predict(fit, df_pred,type = "response"), type = "l",
  ...
  grid()
legend("left",legend = unique(filter_df$ADGENH42), col = 1:5, lty = 1:5, lwd = 2, cex = 0.6)
```
...
```{r}
filter_df$MNHLTH31 = ordered(filter_df$ADGENH42) 
...
# parallel = TRUE imposes proportional odds structure
# 2 intercepts for 3 y categories 
fit <-  vglm(MNHLTH31 ~ AGELAST + SDENICPROD,
             family = cumulative(parallel = TRUE),
             data = filter_df)
...
```{r}
fit2 <-  vglm(MNHLTH31 ~ SDENICPROD + SEX + AGELAST,
             family = cumulative(parallel = TRUE),
             data = filter_df)
pchisq(deviance(fit)-deviance(fit2),
df=df.residual(fit)-df.residual(fit2), lower.tail=FALSE)
