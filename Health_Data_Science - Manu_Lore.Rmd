...
We decide to start with the smoking predictor. We want to keep only *AGE*, *SEX* and *AGEGROUP* as other predictors. Using predictors without interations we can detect better the statistical significance of the model. 
...
but we can see that for some models there is a direct correlation with *AGE*. To prove this 
we can try if some physical deseases have some of connection.
...
matplot(df_pred$AGELAST, predict(fit, df_pred,type = "response"), type = "l",
  ...
  grid()
legend("left",legend = unique(filter_df$MNHLTH31), col = 1:5, lty = 1:5, lwd = 2, cex = 0.6)
```
...
It reminds me a name of a music instrument brand...nevermind.

The Hauck-Donner effect is a problem that can occur if we use small dataframe, indicates that
the significance of our model can be false. But hou can we solvit? Don't worry, is very simple.

We bootstrapped the original dataframe for 100 times (better more but our pc are not so
powerful).
...
```
Fortunatly the boostrap confirm the robustness of our model. 
